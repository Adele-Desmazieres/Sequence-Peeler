...
def unequal_cut(seqs, specie, exe, desired_output, imin, imax, flag_begining) :
	seq = seqs[specie]
	tmp_seqs = seqs.copy()
	imid = (imin + imax) // 2
	...
	if (check_output(tmp_seqs, exe, desired_output)) :
		if flag_begining :
			return unequal_cut(tmp_seqs, specie, exe, desired_output, imid, imax, flag_begining)
		else : 
			return unequal_cut(tmp_seqs, specie, exe, desired_output, imin, imid+1, flag_begining)
...
def dichotomy_cut_one_seq_iter(seqs, specie, exe, desired_output) : # TODO suppr seq
	...
	while not found :
		...
		if check_output(seqs, exe, desired_output) :
			seqs = dichotomy_cut_one_seq_iter(seqs, sp0, seq00, exe, desired_output)
			seqs = dichotomy_cut_one_seq_iter(seqs, sp1, seq00, exe, desired_output)
			continue
		...
		seq3 = seq
		seqs[specie] = seq3
		seq3 = unequal_cut(seqs, specie, exe, desired_output, 0, len(seq3)//2, True)
		seqs[specie] = seq3
		seq3 = unequal_cut(seqs, specie, exe, desired_output, 0, len(seq3), False)
		seqs[specie] = seq3
		if seq3 == seq :
			found = True
...
def dichotomy_cut(seqs, exe, desired_output) :
	...
	for specie,seq in seqs.items() :
		...
		# otherwise reduces the sequence
			...
			cutted_seqs = dichotomy_cut_one_seq_iter(cutted_seqs, specie, exe, desired_output)
